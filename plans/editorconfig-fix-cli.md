...
- Skip files outside the supported UTF-8 text scope.
...
- `--eol`: apply `end_of_line` behavior.
- `--bom`: remove a UTF-8 BOM when `charset` resolves to `utf-8`.
- `--trim`: apply `trim_trailing_whitespace` behavior.
- `--final`: apply `insert_final_newline` behavior.

Validation:

- `file-path` is required and must name an existing file.
- Directories are rejected.
- At least one fix option must be specified: `--all`, `--eol`, `--bom`, `--trim`, or `--final`.
...
- Without `--any`, skip the file unless at least one matching section has a glob other than `*`.
- With `--any`, allow settings from `[*]` alone to drive fixes.

This means a file matched by both `[*]` and `[*.cs]` is eligible without `--any`, and the resolved combined configuration still decides the selected settings.
...
Binary or unsupported-encoding files are skipped and left untouched.
...
- If `charset` is `utf-8`, decode as strict UTF-8 and write without adding a BOM unless the original BOM is intentionally preserved by a non-`--bom` run.
...
- If `charset` is absent, accept UTF-8 with or without a BOM and preserve the original BOM shape unless `--bom` removes it.
- If `charset` is any other value, skip the file.

If UTF-8 decoding fails, skip the file.

Do not treat this tool as a general charset fixer. Except for `--bom`, preserve the original UTF-8 BOM shape as much as possible.
...
- Preserve whether the file ended with a line break unless `--final` is also selected.
...
- `skipped <path>: binary file` when skipped because the file could not be decoded as supported text.
- `skipped <path>: unsupported charset` when skipped because the resolved charset is not `utf-8` or `utf-8-bom`.
- `skipped <path>: only [*] matched` when skipped because `--any` was not supplied.
...
- Settings are not applied when only `[*]` matched and `--any` is absent.
- Settings are applied from `[*]` when `--any` is present.
...
- Explicit non-UTF-8 `charset` values are skipped.

Transform behavior:

- `--bom` removes a UTF-8 BOM only when charset is `utf-8`.
- `--eol` converts LF, CRLF, and CR to the configured style.
- `--trim` removes spaces and tabs before line breaks and EOF only when configured true.
- `--final` with `insert_final_newline = true` adds one final newline to non-empty files.
- `--final` with `insert_final_newline = true` does not add a newline to empty files.
- `--final` with `insert_final_newline = false` makes no change.
...
- Document `--any`, especially the default skip for files matched only by `[*]`.
- Document binary detection and skipped unsupported files.
...
3. Implement EditorConfig discovery, `--git-root`, setting resolution, and `--any` eligibility.
4. Implement UTF-8-only text detection, text loading, unsupported-charset handling, and no-touch byte comparison.
5. Implement `--bom`, `--eol`, `--trim`, and `--final`.
